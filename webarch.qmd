...
### Pair activity - Plug-n-play packages

Many R packages have been developed that "wrap" API queries and format the response. Lucky us! 
In Pairs, let's have a look at `tidycensus`. To get started working with tidycensus, users should load the package along with the tidyverse package, and set you Census API key. A key can be obtained from http://api.census.gov/data/key_signup.html.
